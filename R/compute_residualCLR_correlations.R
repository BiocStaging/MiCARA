...
#' ))
#'
#' # Inject strong signal so features pass significance testing
#' taxa_mat["Taxon_1", 11:20] <- taxa_mat["Taxon_1", 11:20] + 300
#' path_mat["Pathway_1", 11:20] <- path_mat["Pathway_1", 11:20] + 500
#'
#' meta_df <- data.frame(
...
.clr_transform <- function(mat, pseudocount = NULL, label = "matrix") {
    ...
    if (is.null(rownames(mat)) || any(rownames(mat) == "")) {
        stop("The ", label, " matrix must have non-empty feature names.", call. = FALSE)
    }
    if (is.null(colnames(mat)) || any(colnames(mat) == "")) {
        stop("The ", label, " matrix must have non-empty sample IDs.", call. = FALSE)
    }
    if (anyDuplicated(rownames(mat))) {
        stop("The ", label, " matrix contains duplicated feature names.", call. = FALSE)
    }
    if (anyDuplicated(colnames(mat))) {
        stop("The ", label, " matrix contains duplicated sample IDs.", call. = FALSE)
    }
    if (anyNA(mat) || any(!is.finite(mat))) {
        stop("The ", label, " matrix contains missing or non-finite values.", call. = FALSE)
    }
    if (any(mat < 0)) {
        stop("The ", label, " matrix contains negative values and cannot be CLR-transformed.", call. = FALSE)
    }
